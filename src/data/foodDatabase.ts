...
export const KNOWN_LOCAL_IMAGES = new Set([
  'de_mcd_mcplant',
  'de_mcd_bigtastybacon',
  'de_mcd_mcrip',
  'de_mcd_bigmac',
  'de_mcd_royalkaese',
  'de_mcd_royalts',
  ...
  'de_mcd_pommes',
  'de_mcd_mcnuggets',
  'de_mcd_gitterkartoffeln',
  'de_mcd_netzkartoffeln',
  'de_mcd_gartensalat',
  'de_mcd_cola_zero',
  'de_mcd_cola',
  'de_mcd_fanta',
  'de_mcd_kaffee_schwarz',
  'de_mcd_suesssauer',
  'de_mcd_bbq',
  'de_mcd_ketchup',
  'de_mcd_mayo',
  'de_mcd_sourcream',
  'de_mcd_mcmuffin_bacon',
  'de_mcd_mcmuffin_sausage',
  'de_mcd_mcmuffin_egg'
]);

export function getLocalImageUrl(id: string): string {
  if (KNOWN_LOCAL_IMAGES.has(id)) {
...
export const FOOD_DATABASE: MenuItem[] = [
  // ================= Klassiker (主食) =================
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcplant'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_bigtastybacon'),
    atoms: [
  ...
  },
  {
    ...
    category: 'klassiker',
    base_calories: 485,
    description: 'Saftiges Schweinefleischpatty, würzige McRip-Soße, knackige Zwiebeln und Gewürzgurken auf einem länglichen Brötchen.',
    imageUrl: getLocalImageUrl('de_mcd_mcrip'),
    atoms: [
      { id: 'mcrip_bun', name: '麦肋排长面包 (McRip-Brötchen)', type: 'carb', calories: 175, protein: 6.2, fat: 2.6, carbs: 32, salt: 0.56, removable: false, default: true },
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_bigmac'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_royalkaese'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_royalts'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcchicken'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_filetofish'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_triplecheese'),
    atoms: [
  ...
  // ================= Snacks & Beilagen (小食/配菜) =================
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_pommes'),
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcnuggets'),
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_gitterkartoffeln'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_netzkartoffeln'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_gartensalat'),
    atoms: [
  ...
  // ================= Getränke (饮料) =================
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_cola_zero'),
    supportedSizes: ['S', 'M', 'L'],
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_cola'),
    supportedSizes: ['S', 'M', 'L'],
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_fanta'),
    supportedSizes: ['S', 'M', 'L'],
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_kaffee_schwarz'),
    supportedSizes: ['M', 'L'],
    atoms: [
  ...
  // ================= Saucen & Dips (酱料/蘸酱) =================
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_suesssauer'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_bbq'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_ketchup'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mayo'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_sourcream'),
    atoms: [
  ...
  // ================= Frühstück (早餐/麦满分) =================
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcmuffin_bacon'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcmuffin_sausage'),
    atoms: [
  ...
  },
  {
    ...
    imageUrl: getLocalImageUrl('de_mcd_mcmuffin_egg'),
    atoms: [
...
export const CACHE_KEY = "mcd_de_food_db_v4";
...
try {
  ...
  localStorage.removeItem('mcd_de_food_db_v2');
  localStorage.removeItem('mcd_de_food_db_v3');
} catch (e) {
  // Silent catch for non-browser environments or security blocks
}

export const CACHE_TIME_KEY = "mcd_de_food_db_last_fetch";
export const CACHE_INTERVAL = 24 * 60 * 60 * 1000; // 24 hours
...
export async function initializeDatabase(): Promise<MenuItem[]> {
  try {
    // Check if we fetched within the last 24 hours
    const lastFetch = localStorage.getItem(CACHE_TIME_KEY);
    if (lastFetch) {
      const lastFetchTime = parseInt(lastFetch, 10);
      if (!isNaN(lastFetchTime) && Date.now() - lastFetchTime < CACHE_INTERVAL) {
        console.log('Database loaded from local cache (within 24-hour TTL).');
        return getCachedDatabase();
      }
    }

    console.log('Cache expired or empty. Fetching remote database...');
    ...
    if (Array.isArray(latestData) && latestData.length > 0 && latestData[0].id) {
      // Map remote items to use local images if they match our known local image ids
      const mappedData = latestData.map((item: MenuItem) => ({
        ...item,
        imageUrl: getLocalImageUrl(item.id) || item.imageUrl
      }));
      try {
  ...
  try {
    ...
    if (cached) {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        // Map cached items to use local images if they match our known local image ids
